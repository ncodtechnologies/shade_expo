  ...
  render() {
    const tableRows = this.state.arrLedgers.map((arrLedger, index) => (
      <TableRow
        ...
        loadLedgerList={this.loadLedgerList}
      />
    ));
                                ...
                                <div class="col-sm-4">
                                  <div class="form-group">
                                    ...
                                    <select
                                      class="form-control"
                                      onChange={(e) =>
                                        this.onLedgerGroupChange(e)
                                      }
                                      value={this.state.ledgerGroup}
                                    >
                                      <option value="">All</option>
                                      {this.state.arrLedgerGroup.map(
                                        (types) => (
                                          <option value={types.id_ledger_group}>
                                            {types.name}
                                          </option>
                                        )
                                      )}
                                ...
                                <div class="col-sm-8">
                                  <div class="form-group float-right">
                                    ...
                                    >
                                      <button
                                        type="submit"
                                        class="btn btn-block btn-success btn-fla"
                                      >
                                        Create
                                      </button>
                                    </Link>
      ...
      buttons: [
        {
          label: "Yes",
          onClick: () => {
            fetch(URL_LEDGER_DELETE + `/${id_account_head}`).then(
              (response) => {
                response.json();
                const id_ledger_group = this.props.ledgerGroup;
                const activePage = this.props.activePage;
                this.props.loadLedgerList(id_ledger_group, activePage);
              }
            );
          },
        },
        ...
        <td>
          <div class="btn-group">
            <a
              ...
              onClick={(e) => {
                e.preventDefault();
                this.delLedger(arrLedger.id_account_head);
              }}
            >
            ...
            <Link
              ...
              to={"./ledgerCreate/" + arrLedger.id_account_head}
              render={(props) => (
                <LedgerCreate
                  {...props}
                  id_ledger={this.props.match.params.id_ledger}
                />
              )}
            >
